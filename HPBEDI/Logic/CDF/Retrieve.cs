...
namespace DataHPBEDI.Logic.CDF
{
	public class Retrieve : IDisposable
	{
		...
		public (Direct.PurchaseOrder.Header header, List<Direct.PurchaseOrder.Detail> details) PurchaseOrder(string ordernumber)
		{
			Direct.PurchaseOrder.Header header = null;
			List<Direct.PurchaseOrder.Detail> details = null;
			DataSet data = null;
			try
			{
				using (Access read = new Access())
				{
					...
					(
						...
						new List<string>() { "CDF.uspPurchaseOrder_Retrieve" },
						...
						if (data.Tables.Count == 2)
						{
							if (data.Tables[0] != null && data.Tables[0].Rows.Count == 1)
							{
								header = (from DataRow row in data.Tables[0].Rows
										  select new Models.CDF.PurchaseOrder.Header()
										  {
							...
							if (data.Tables[1] != null && data.Tables[1].Rows.Count > 0)
							{
								details = (from DataRow row in data.Tables[1].Rows
										   select new Models.CDF.PurchaseOrder.Detail
										   {
