CREATE PROCEDURE [EDI].[uspInvoice_Insert]
(
	 @header AS EDI.TypeInvoiceHeader READONLY
	,@detail AS EDI.TypeInvoiceDetail READONLY
	,@ediver AS TINYINT
)
AS
BEGIN
	...
	BEGIN TRY
		INSERT INTO edi.InvoiceHeader ([PONumber], [InvoiceNo], [IssueDate], [VendorId], [ReferenceNo], [ShipToLoc], [ShipToSAN], [BillToLoc], [BillToSAN], [ShipFromLoc], [ShipFromSAN], [TotalLines], [TotalQuantity], [TotalPayable], [CurrencyCode], [InsertDateTime], [Processed], [ProcessedDateTime], [InvoiceACKSent], [InvoiceAckNo], [GSNo], [EDISourceTypeId])
		...
		IF EXISTS(SELECT 1 FROM @detail d INNER JOIN #inserted i ON d.[ponumber] = i.po)
			BEGIN
				...
				INSERT INTO edi.InvoiceDetail ([InvoiceId], [LineNo], [ItemIdCode], [ItemIdentifier], [ItemDesc], [InvoiceQty], [UnitPrice], [DiscountPrice], [DiscountCode], [DiscountPct], [RetailPrice])
