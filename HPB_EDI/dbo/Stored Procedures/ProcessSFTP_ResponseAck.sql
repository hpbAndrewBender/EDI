...
-- =============================================
CREATE PROCEDURE [dbo].[ProcessSFTP_ResponseAck]
AS
...
select distinct h.AckID,h.PONumber,v.ParentFolder,
		...
		+'ST*997*000000001~'
		+CHAR(13) + CHAR(10)
		+'AK1*PR*'+CONVERT(varchar(12),case h.ShipFromSAN when '2002442' then h.ReferenceNo when '8600023' then isnull(h.GSNo,'') else h.PONumber end)+'~'
		+CHAR(13) + CHAR(10)
...
where h.ResponseACKSent=0 and v.processor='SFTP' and h.VendorID in (select VendorID from HPB_EDI..Vendor_SAN_Codes where ACK997=1)
