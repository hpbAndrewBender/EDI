...
BEGIN
	...
	insert into #vends
		...
		from [850_PO_Hdr] h with(nolock) 
			inner join [850_PO_Dtl] d with(nolock)	
				on h.OrdID=d.OrdID
	...
	--build order string....
		...
		----update next
		if (select count(distinct POnumber) from [850_PO_Hdr] where Processed=0 and VendorID=@curVend and PONumber not in (select distinct PONumber from #ords))=0
			begin
