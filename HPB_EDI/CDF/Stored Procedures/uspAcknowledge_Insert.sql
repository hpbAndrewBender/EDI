...
(
	 ...
	 @header AS CDF.TypeAcknowledgeHeader READONLY
	,@detail AS CDF.TypeAcknowledgeDetail READONLY
	,@ediver AS TINYINT
)
AS
BEGIN
	...
	BEGIN TRY
		INSERT INTO CDF.AcknowledgeHeader ([PONumber], [IssueDate], [VendorId], [ReferenceNo], [ShipToLoc], [ShipToSAN], [BillToLoc], [BillToSAN], [ShipFromLoc], [ShipFromSAN], [TotalLines], [TotalQuantity], [CurrencyCode], [InsertDateTime], [Processed], [ProcessedDateTime], [ResponseACKSent], [ResponseAckNo], [GSNo], [EDISourceTypeId], [VendorMessage])
		...
		IF EXISTS(SELECT 1 FROM @detail d INNER JOIN #inserted i on d.[ponumber] = i.po)
			BEGIN
				...
				INSERT INTO CDF.AcknowledgeDetail ([AckId], [LineNo], [LineStatusCode], [ItemStatusCode], [UnitOfMeasure], [QuantityOrdered], [QuantityShipped], [QuantityCancelled], [QuantityBackordered], [UnitPrice], [PriceCode], [CurrencyCode], [ItemIdCode], [ItemIdentifier], [ItemDesc], [EDIFileID], [EDILineNumber], [VendorStatus])
