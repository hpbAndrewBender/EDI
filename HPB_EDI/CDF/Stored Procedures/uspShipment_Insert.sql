...
(
	 @header AS CDF.TypeShipmentHeader READONLY
	,@detail AS CDF.TypeShipmentDetail READONLY
	,@ediver AS TINYINT
)
AS
BEGIN
	...
	BEGIN TRY
		INSERT INTO CDF.ShipmentHeader ([PONumber], [ASNNo], [IssueDate], [VendorID], [ReferenceNo], [ShipToLoc], [ShipToSAN], [BillToLoc], [BillToSAN], [ShipFromLoc], [ShipFromSAN], [Carrier], [TotalLines], [TotalQuantity], [CurrencyCode], [InsertDateTime], [Processed], [ProcessedDateTime], [ASNACKSent], [ASNAckNo], [GSNo], [EDISourceTypeId])
		...
		IF EXISTS(SELECT 1 FROM @detail d INNER JOIN #inserted i ON d.[ponumber] = i.po)
			BEGIN
				...
				INSERT INTO CDF.ShipmentDetail ([ShipmentID], [LineNo], [ItemIdCode], [ItemIdentifier], [ItemDesc], [QuantityShipped], [PackageNo], [TrackingNo], [ReferenceNumber])
					SELECT i.id, [LineNo], [ItemIdCode], [ItemIdentifier], [ItemDesc], [QuantityShipped], [PackageNo], [TrackingNo], ReferenceNumber
					FROM @detail d
